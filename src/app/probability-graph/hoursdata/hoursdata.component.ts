...
@Component({
  selector: 'app-hoursdata',
  templateUrl: './hoursdata.component.html',
  styleUrls: ['./hoursdata.component.css']
})
export class HoursdataComponent implements OnInit {
  ...
  dynamicGraph(data): any {
    ...
    //   vpnObj = obj[i]
    // }
   // vpnObj= data.text['global_vpncount_by_hour'];
    if(data.text.status==='global'){
      vpnObj= data.text[0]['global_vpncount_by_hour'];
      this.global_alltime_vpncount=data.text[0].global_alltime_vpncount;
    } else{
      vpnObj= data.text['user_vpncount_by_hour'];
