...
@Component({
  selector: 'app-prohour-data',
  templateUrl: './prohour-data.component.html',
  styleUrls: ['./prohour-data.component.css']
})
export class ProhourDataComponent implements OnInit {
  ...
  dynamicGraph(data): any {
    ...
    //   vpnObj = obj[i]
    // }
    //vpnObj= data.text['global_badgecount_by_hour'];
...
private width: number;
  ...
  constructor(private vpnService : VpnDataService,  private container: ElementRef) {
    this.subscription = this.vpnService.getproDtaForHour().subscribe(message => { 
      this.message = message; 
        ...
        //   this.dynamicGraph(message);
        // } else if(this.message.text[0].hasOwnProperty('bucket_hour')&&this.indexes.indexOf("bucket_hour")!==-1){
        //   this.indexes.push('bucket_hour');
        //   this.dynamicGraph(message);
