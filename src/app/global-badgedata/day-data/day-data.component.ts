...
export class DayDataComponent implements OnInit {
  ...
  dynamicGraph(data): any {
    let vpnObj = [];
    $('#badgeDay').empty();
    this.global_alltime_badgecount=data.global_alltime_badgecount;
    // for (let i = 0; i < data.length; i++) {
    //   let obj = [];       
    //   obj[i] = data[i]['global_badgecount_by_day'];
    //   vpnObj = obj[i]
    // }
    d3.select('#badgeDay').selectAll('svg').remove();
...
private width: number;
  ...
  constructor(private vpnService : VpnDataService,  private container: ElementRef) {
    this.subscription = this.vpnService.getDtaForDate().subscribe(message => { 
